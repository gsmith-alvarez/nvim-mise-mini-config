...
-- Extracts capture groups from regTrig matches
-- ============================================================================
...
-- Helper to create "Operator" snippets (e.g., 'in ', 'as ', 'es ')
-- Ensures they only fire with a trailing space and don't collide with longer words.
local make_operator = function(snips, trig, expansion)
  -- Mid-line case: Non-letter before, space after
  ...
  -- Start-of-line case: Space after
  table.insert(snips, s({
      trig = "^" .. trig .. "(%s)",
      regTrig = true,
      wordTrig = false,
...
-- 1. Vault Path Definition
local vault_path = vim.fn.expand("~/Documents/Obsidian")

-- 2. Robust Math Context Detector (Hallucination-Free)
local in_mathzone = function()
  ...
  while node do
    ...
    if node_type == "math_block" or node_type == "inline_formula" or node_type == "math_environment" or
        node_type == "math_shift" or node_type == "displayed_equation" then
      return true
    end
    node = node:parent()
  end
...
local in_vault = function()
  ...
  return buf_name:find(vault_path, 1, true) ~= nil
end

-- 3. Data Tables (Source of Truth)
local GREEK_MAP = {
...
function M.retrieve()
  ...
  local vault_callout = ls.extend_decorator.apply(ls.parser.parse_snippet, {
    condition = pipe({ function() return not in_mathzone() end, in_vault }),
    snippetType = "autosnippet", wordTrig = true,
  })
  ...
  for name, short in pairs(GREEK_MAP) do
    ...
    table.insert(snips,
      s(
        { trig = "([^%s\\])" .. name, regTrig = true, wordTrig = false, snippetType = "autosnippet", condition =
        in_mathzone },
        f(function(_, snip) return snip.captures[1] .. "\\" .. name end)
      ))
    table.insert(snips,
      s(
        { trig = "\\" .. name .. "([A-Za-z])", regTrig = true, wordTrig = false, snippetType = "autosnippet", condition =
        in_mathzone },
  ...
  for trig, exp in pairs(syms) do table.insert(snips, math_auto({ trig = trig }, exp)) end
  ...
  table.insert(snips,
    s(
      { trig = "pa([A-Za-z])([A-Za-z])", regTrig = true, wordTrig = false, snippetType = "autosnippet", condition =
      in_mathzone },
  ...
  for trig, cmd in pairs(decs) do
    table.insert(snips,
      s(
        { trig = "([a-zA-Z])" .. trig, regTrig = true, snippetType = "autosnippet", condition = in_mathzone, wordTrig = false },
        fmt(cmd .. "{{{1}}}", { cap(1) })
      ))
  end
  ...
  -- [CHUNK 15]: VISUAL OPERATIONS (mA)
  table.insert(snips, math_auto({ trig = "U" }, "\\underbrace{ ${1:${TM_SELECTED_TEXT}} }_{ $0 }"))
  table.insert(snips, math_auto({ trig = "O" }, "\\overbrace{ ${1:${TM_SELECTED_TEXT}} }^{ $0 }"))
  table.insert(snips, math_auto({ trig = "B" }, "\\underset{ $1 }{ ${2:${TM_SELECTED_TEXT}} }"))
  table.insert(snips, math_auto({ trig = "C" }, "\\cancel{ ${1:${TM_SELECTED_TEXT}} }"))
  table.insert(snips, math_auto({ trig = "K" }, "\\cancelto{ $1 }{ ${2:${TM_SELECTED_TEXT}} }"))
  table.insert(snips, math_auto({ trig = "S" }, "\\sqrt{ ${1:${TM_SELECTED_TEXT}} }"))

  -- [CHUNK 16]: MATRIX ENVIRONMENTS (Appendages)
  ...
  -- [CHUNK 17]: STRUCTURAL ENVIRONMENTS (Appendages)
  ...
  -- [CHUNK 18]: BRACKETS & ENCLOSURES (Appendages)
  local bracket_map = {
    avg = { "\\langle ", " \\rangle" },
  ...
  -- [CHUNK 19]: AUTO-ENCLOSURES (Global Appendages)
  ...
  -- [CHUNK 20]: TEXT-TO-MATH CONVERSIONS (Complex Regex)
  -- 1. Standalone letters -> math (e.g., ' n ' -> ' $n$ ')
  table.insert(snips, s({ trig = "([^']) ([B-HJ-Zb-z]) ([%s.,?!:'])", regTrig = true, snippetType = "autosnippet" },
    fmt("{1}${2}${3}", { cap(1), cap(2), cap(3) })
  ))
  -- 2. Greek in text -> math (e.g., 'alpha ' -> '$\alpha$ ')
  ...
  -- 3. Equations in text -> math (e.g., 'x=2 ' -> '$x=2$ ')
